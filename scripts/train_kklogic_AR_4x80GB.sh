...
EXP_NAME="Qwen2.5-7B-Instruct-1M-kklogic_grpo_AR"
MAX_PROMPT=400
MICRO_BS=32
VAL_N=16
TRAIN_BS=64
...
REWEIGHT_K=0.2  # alpha in the paper; Note that alpha=0.0 means no reweighting
REWEIGHT_TAU=0.8   # (1 - alpha) in the paper
NEG_ADV_WEIGHT=1.0

# ! Low-Probability Token Isolation (Lopti)
SEPERATE_UPDT=False  # Wether to use separate updating for low-probability tokens
SEPERATE_PROB=0.5   # eta in the paper; Note that positive values means updating low-prob token first, negative values means updating high-prob tokens first.
