...
from verl.trainer.ppo.ray_evaluator import RayEvaluator
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            if already_print_data_sources[data_source] < self.num_examine:
                ...
                print(sequences_str.rstrip("<|endoftext|>"))
...
def main(config):
    if not ray.is_initialized():
        ...
        ray.init(runtime_env={'env_vars': {'TOKENIZERS_PARALLELISM': 'true', 
                                           ...
                                           "WANDB_API_KEY":"7613259832568f1a899d34b6d58f8c4e9f18353f"}})

    # ray.get(main_task.remote(config))
    main_task(config)

# note: only call 1 cpu core and no gpu cores
# @ray.remote
def main_task(config):
    ...
    from verl.trainer.ppo.ray_evaluator import ResourcePoolManager, Role
    ...
    evaluator = RayEvaluator(config=config,
                            tokenizer=tokenizer,
