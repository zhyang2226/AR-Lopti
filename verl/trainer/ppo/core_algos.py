...
# def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange_low, cliprange_high):
...
def kl_penalty(logprob: torch.FloatTensor, ref_logprob: torch.FloatTensor, kl_penalty) -> torch.FloatTensor:
    ...
    if kl_penalty == 'low_var_kl':
        ...
        kl = torch.clamp(kl, min=-5, max=5) # limit the range to avoid numerical issues
        ratio = torch.exp(kl)
