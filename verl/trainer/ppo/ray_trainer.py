...
class RayPPOTrainer(object):
    ...
    #     reward_tensor_lst = []
    #     data_source_lst = []
    #     for test_data in self.amc_dataloader:
    #         test_batch = DataProto.from_single_dict(test_data)
    ...
    #     metric_dict = {}
    #     for data_source, rewards in data_source_reward.items():
    #         count_equal_max = sum(1 for reward in rewards if reward == 1)
    #         total_count = len(rewards)
    #         metric_dict[f'val/test_score/{data_source}'] = count_equal_max / total_count if total_count > 0 else 0
    ...
    #     reward_tensor_lst = []
    #     data_source_lst = []
    #     for test_data in self.aime_dataloader:
    #         test_batch = DataProto.from_single_dict(test_data)
    ...
    #         test_gen_batch_padded, pad_size = pad_dataproto_to_divisor(test_gen_batch, self.actor_rollout_wg.world_size)
    #         if n_val_samples > 1:
    #             test_gen_batch_padded.meta_info['val_temperature'] = 0.6
    #         test_output_gen_batch_padded = self.actor_rollout_wg.generate_sequences(test_gen_batch_padded)
    #         # unpad
    #         test_output_gen_batch = unpad_dataproto(test_output_gen_batch_padded, pad_size=pad_size)
    #         print('validation generation end')
    ...
    #     reward_tensor = torch.cat(reward_tensor_lst, dim=0).sum(-1).cpu().reshape(-1, n_val_samples)  # (batch_size,)
    #     data_sources = np.concatenate(data_source_lst, axis=0).reshape(-1, n_val_samples)
    #     # evaluate test_score based on data source
    #     data_source_reward = {}
    #     for i in range(reward_tensor.shape[0]):
