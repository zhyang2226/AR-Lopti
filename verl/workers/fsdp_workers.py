...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        # normalize config
        print(f"Initial ppo_mini_batch_size: {self.config.actor.ppo_mini_batch_size}")
        print(f"Initial ppo_micro_batch_size: {self.config.actor.ppo_micro_batch_size}")
        print(f"self.device_mesh.shape[0]: {self.device_mesh.shape[0]}")
        print(f"self.ulysses_sequence_parallel_size: {self.ulysses_sequence_parallel_size}")
        print(f"self.config.rollout.n: {self.config.rollout.n}")
        if self._is_actor:
    ...
    def _build_model_optimizer(self,
        ...
        if self._is_actor:
            ...
            if self.config.actor.optim.type == 'adamw':
                actor_optimizer = optim.AdamW(actor_module_fsdp.parameters(),
                                            lr=optim_config.lr,
                                            betas=optim_config.get('betas', (0.9, 0.999)),
                                            weight_decay=optim_config.get('weight_decay', 1e-2))
            elif self.config.actor.optim.type == 'sgd':
                actor_optimizer = optim.SGD(actor_module_fsdp.parameters(),
                                            lr=optim_config.lr,)
                                            # momentum=optim_config.get('momentum', 0.9),
                                            # weight_decay=optim_config.get('weight_decay', 1e-2))
            elif self.config.actor.optim.type == 'RMSProp':
                actor_optimizer = optim.RMSprop(actor_module_fsdp.parameters(),
                                                lr=optim_config.lr,
                                                alpha=optim_config.get('alpha', 0.99),
                                                eps=optim_config.get('eps', 1e-8),
                                                weight_decay=optim_config.get('weight_decay', 1e-2))

            else:
                raise ValueError(f'Unsupported optimizer type: {self.config.actor.optim.type}')
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        if 'retemp_temperature' in data.batch:
            # print(f'Using retemp_temperature: {data.batch["retemp_temperature"]}')
            data.meta_info['temperature'] = data.batch['retemp_temperature']
        else:
            # print(f'Using config temperature: {self.config.rollout.temperature}')
    ...
    # For re-compute the old_log_prob, we add a new function here
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
